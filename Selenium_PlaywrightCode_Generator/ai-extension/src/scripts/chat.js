...
class ChatUI {
    ...
    getPromptKey(language, engine) {
        // If it's TypeScript+Playwright, we do: check for page/test mode
        if (language === 'typescript' && engine === 'playwright') {
        //  return 'PLAYWRIGHT_CODE_GENERATION';
        const selectedRadio = document.querySelector('input[name="playwrightGenerationMode"]:checked');
        return selectedRadio?.value === 'PAGE' 
