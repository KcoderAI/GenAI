...
PLAYWRIGHT_PAGE_ONLY: `
    ...
    Generate a Playwright Page Object Model class in TypeScript for this page.
    Action to perform: \${userAction}
    URL: \${pageUrl}
  ...
  Requirements:
    ...
    5. Assertion guidelines:
       - Always use locators for assertions (e.g., getByText, getByRole)
       - Never use assertions directly on the page object (e.g., expect(page).toContainText())
       - Use proper locator-based assertions like expect(locator).toHaveText() or expect(locator).toBeVisible()
       - For text assertions, use toHaveText() or toContainText() on a locator, not on the page object
       - Respond with only the complete code block and no other text.
